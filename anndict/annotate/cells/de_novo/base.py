...
#     if n_categories > 50:
#         warnings.warn(f"The '{groupby}' column has {n_categories} groups, which \
#             may result in slow runtimes. Ensure that {groupby} is not continuous data.", UserWarning)
...
#         print(f"rerunning diffexp analysis because not found \
#             in adata.uns for adata.obs['{groupby}']. (run before annotating to avoid this)")
...
#         warnings.warn(f"Tissue of origin column '{tissue_of_origin_col}' not found in adata.obs, \
#             will not consider tissue of origin for cell type annotation.", UserWarning)
...
#         warnings.warn(f"Cell type of origin column '{cell_type_of_origin_col}' not found in \
#             adata.obs, will not consider cell type of origin for annotation.", UserWarning)
...
def _validate_and_prepare_rank_genes(adata, groupby):
    """
...
def _validate_metadata_columns(adata, tissue_col=None, cell_type_col=None):
    """
    ...
    -------
    tuple
        (tissue_col, cell_type_col) - Returns None for invalid columns
    """
...
def _get_cluster_metadata(adata, clusters, groupby, tissue_col=None, cell_type_col=None):
    """
    ...
    -------
    tuple
        (cluster_to_tissue, cluster_to_cell_type)
    """
    cluster_to_tissue = {}
    cluster_to_cell_type = {}
...
def _update_adata_and_create_results(adata, cell_type_annotations, groupby, new_label_column, n_top_genes, top_genes_lists=None):
    """
    ...
    top_genes_lists : dict or list, optional
...
def ai_annotate(
    ...
    **kwargs,
    ...
    This uses marker genes for each cluster and applies func to determine the label 
    for each cluster based on the top n marker genes. The results are added to the 
    AnnData object and returned as a DataFrame.
    ...
    n_top_genes : int
        The number of top marker genes to consider for each cluster
    new_label_column : str
        The name of the new column in adata.obs where annotations will be stored
    tissue_of_origin_col : str, optional
        Name of column in adata.obs containing tissue of origin
    **kwargs
        Additional kwargs passed to func
...
def ai_annotate_by_comparison(
    ...
    **kwargs,
    ...
    ----------
    func : callable
        A function that takes gene_lists: list[list[str]] and returns annotations: list[str]
    adata : AnnData
        An AnnData object
    groupby : str
        Column in adata.obs to group by for differential expression analysis
    ...
    new_label_column : str
        The name of the new column in adata.obs where annotations will be stored
    cell_type_of_origin_col : str, optional
        Name of column in adata.obs containing cell type of origin
    tissue_of_origin_col : str, optional
        Name of column in adata.obs containing tissue of origin
    **kwargs
        Additional kwargs passed to func
