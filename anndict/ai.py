...
def ai_compare_cell_types_binary(label1, label2):
    ...
    # Prepare the messages for the Chat Completions API
    messages = [
        {"role": "system", "content": "You are a terse molecular biologist who determines if two labels refer to the same cell type. Respond only with 'yes' or 'no'. Examples: 1) CD8-positive T cell 2) T cell -> yes; 1) epithelial cell 2) intrahepatic cholangiocyte -> yes; 1) Macrophage 2) Endothelial Cell -> no"},
        {"role": "user", "content": gpt_prompt}
    ]
...
def ai_resolution_interpretation(plot):
    """
    Determines the clustering resolution adjustment based on an image of a plot using the OpenAI API.
