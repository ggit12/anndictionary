...
def set_var_index_func(
    ...
    column
        The column name(s) to set as the index of ``adata.var``.

    Returns
    --------
    An :class:`AdataDict` with the same structure as ``adata_dict``, where the ``.var`` attribute of each :class:`AnnData` object has its index set to the specified column.
    """
...
def set_obs_index_func(
    ...
    column
        The column name(s) to set as the index of ``adata.obs``.

    Returns
    --------
    An :class:`AdataDict` with the same structure as ``adata_dict``, where the ``.obs`` attribute of each :class:`AnnData` object has its index set to the specified column.
    """
